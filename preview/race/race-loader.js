(() => {
  ...
  const fail = error => {
    ...
    if (loading) loading.textContent = "Pass-and-play could not start. Please reload the page.";
  };
  ...
  const start = async () => {
    ...
    try {
      ...
      await loadScript(`preview/race/race.js?v=${VERSION}`);
      if (!window.__NEARER_RACE_STARTED) throw new Error("The race game did not initialise.");
    } finally {
