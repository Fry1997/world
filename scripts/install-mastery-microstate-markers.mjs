...
replaceRequired(
  ...
  'function hitCountry(clientX, clientY) { const rect = elements.stage.getBoundingClientRect(); const point = [clientX - rect.left, clientY - rect.top]; let nearestMarker = null, nearestDistance = Infinity; for (const marker of microstateMarkers) { const distance = Math.hypot(marker.point[0]-point[0],marker.point[1]-point[1]); if (distance <= marker.hitRadius && distance < nearestDistance) { nearestMarker = marker; nearestDistance = distance; } } if (nearestMarker) return nearestMarker.code; const translate = projection.translate(); const radiusValue = projection.scale(); if (Math.hypot(point[0]-translate[0],point[1]-translate[1]) > radiusValue) return null; const coordinate = projection.invert(point); if (!coordinate) return null; const feature = polygonFeatures.find(item => d3.geoContains(item, coordinate)); return feature?.properties.code || null; }',
